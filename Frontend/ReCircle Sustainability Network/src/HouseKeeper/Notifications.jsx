        ...
        if (res.ok && Array.isArray(data)) {
          const realNotifs = data
            ...
            .map((req) => ({
              ...
              message:
                req.status === "Pending"
                  ? `Your request for ${req.quantity}kg ${req.wasteType} is waiting for a collector.`
                  : req.status === "Accepted"
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white w-full max-w-[90%] sm:max-w-sm rounded-3xl p-5 sm:p-6 shadow-2xl relative animate-in fade-in zoom-in duration-200">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button
                onClick={() =>
                  updateRequestStatus(selectedNotif.id, "Cancelled")
                }
              ...
              <button
                onClick={() =>
                  updateRequestStatus(selectedNotif.id, "Delivered")
                }
...
const LeafIcon = () => (
  <svg
    ...
    className="text-sky-600 sm:w-5 sm:h-5"
  >
