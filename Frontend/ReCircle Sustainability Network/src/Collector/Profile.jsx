...
const Profile = () => {
  ...
  const totalCollections = 148; // Total Pickups completed
  const totalWeightCollected = "1.2 Ton"; // Total Waste Processed

  // Collector Badge System based on number of Collections
  const getCollectorBadge = (collections) => {
    if (collections >= 500) return { icon: "💎", name: "Apex Logistics Partner", bg: "bg-amber-100", text: "text-amber-700", border: "border-amber-200" };
    if (collections >= 200) return { icon: "👑", name: "Master Collector", bg: "bg-blue-100", text: "text-blue-700", border: "border-blue-200" };
    if (collections >= 100) return { icon: "⚡", name: "Pro Fleet Partner", bg: "bg-emerald-100", text: "text-emerald-700", border: "border-emerald-200" };
    if (collections >= 50) return { icon: "🟢", name: "Reliable Collector", bg: "bg-teal-100", text: "text-teal-700", border: "border-teal-200" };
    return { icon: "🚛", name: "Eco Carrier", bg: "bg-green-100", text: "text-green-700", border: "border-green-200" };
  };
    ...
    <div className="w-full max-w-[1400px] mx-auto px-5 md:px-8 lg:px-12 lg:h-[calc(100vh-110px)] font-sans pb-10 -mt-2 sm:-mt-0">
      <div className="flex flex-col lg:flex-row gap-5 lg:gap-6 h-full">
        ...
        <div className="w-full lg:w-[340px] xl:w-[360px] flex-shrink-0 flex flex-col bg-white rounded-[24px] border border-gray-200 shadow-sm overflow-hidden h-fit">
          ...
          {/* Cover Photo - Truck / Logistics Theme */}
          ...
          <div className="px-5 pb-5 relative flex flex-col items-center sm:items-start text-center sm:text-left shrink-0 border-b border-gray-100">
            ...
            <div className="relative -mt-12 sm:-mt-14 mb-3">
              <div className="w-24 h-24 sm:w-20 sm:h-20 rounded-full border-[3px] border-white shadow-md overflow-hidden bg-gradient-to-br from-[#064e3b] to-emerald-600 flex items-center justify-center">
                {profileImage ? (
                  ...
                  <span className="text-white text-3xl font-bold tracking-wider drop-shadow-sm">RV</span>
                )}
              </div>
            ...
            <div className={`mt-3 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border ${collectorBadge.bg} ${collectorBadge.text} ${collectorBadge.border} font-semibold text-[11px] uppercase tracking-wider shadow-sm`}>
              <span className="text-sm">{collectorBadge.icon}</span> {collectorBadge.name}
            </div>
          </div>

          {/* Impact Stats Grid - Rewritten for Collector domain */}
        ...
        <div className="flex-1 flex flex-col bg-white rounded-[24px] border border-gray-200 shadow-sm overflow-hidden h-fit lg:h-full">
          ...
          <div className="flex justify-between items-center px-5 sm:px-8 py-5 sm:py-6 border-b border-gray-100 bg-gray-50/30 shrink-0">
            <div>
              ...
              <p className="text-gray-500 text-[12px] sm:text-[13px] font-medium mt-0.5">Manage your service info, location, and alerts.</p>
            </div>
            {isEditing && (
          ...
          <div className="p-4 sm:p-8 overflow-y-auto flex-1 space-y-3 lg:space-y-10 scrollbar-hide bg-gray-50/50 lg:bg-white">
            ...
            <section className="animate-fadeIn bg-white lg:bg-transparent border border-gray-200 lg:border-none rounded-2xl lg:rounded-none overflow-hidden lg:overflow-visible shadow-sm lg:shadow-none">
              ...
              >
                <h3 className="text-[14px] lg:text-[15px] font-bold text-gray-900 flex items-center gap-2">
                  <Mail size={18} className="text-emerald-600" /> Personal & Service Details
                </h3>
              ...
              <div className={`border-t border-gray-100 lg:border-none ${activeTab === 'personal' ? 'block' : 'hidden lg:block'}`}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 lg:bg-gray-50/50 p-4 lg:p-5 lg:rounded-2xl lg:border border-gray-100/80 lg:mt-5">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-bold text-gray-500 uppercase tracking-wider ml-1">Assigned Zone / City</label>
                    <div className="relative">
            ...
            <section className="animate-fadeIn bg-white lg:bg-transparent border border-gray-200 lg:border-none rounded-2xl lg:rounded-none overflow-hidden lg:overflow-visible shadow-sm lg:shadow-none">
              ...
              >
                <h3 className="text-[14px] lg:text-[15px] font-bold text-gray-900 flex items-center gap-2">
                  <Bell size={18} className="text-emerald-600" /> Operational Alerts & Notifications
                </h3>
              ...
              <div className={`border-t border-gray-100 lg:border-none ${activeTab === 'preferences' ? 'block' : 'hidden lg:block'}`}>
                <div className="lg:bg-gray-50/50 p-4 lg:p-5 lg:rounded-2xl lg:border border-gray-100/80 space-y-3 lg:space-y-4 lg:mt-5">
                  {[
                    { title: "New Pickup Requests", desc: "Get instant alerts for pending waste requests in your area.", defaultChecked: true },
                    { title: "Payout & Earnings Alerts", desc: "Get notified as soon as a collection weight is approved and wallet updates.", defaultChecked: true },
                    { title: "Schedule Reminders", desc: "Alerts for pre-scheduled corporate or residential routine pickups.", defaultChecked: false }
                  ].map((pref, i) => (
