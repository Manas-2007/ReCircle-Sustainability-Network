...
const EcoPoints = () => {
  ...
  const badgeData = [
    ...
    { icon: "🌿", name: "Green Warrior", level: 3, threshold: 1000 },
    { icon: "🌍", name: "Planet Protector", level: 4, threshold: 2000 },
    { icon: "👑", name: "Champion", level: 5, threshold: 5000 },
  ];

  // Find Current Level Index based on totalPoints
  let currentLevelIndex = 0;
  for (let i = 0; i < badgeData.length; i++) {
    if (totalPoints >= badgeData[i].threshold) {
      currentLevelIndex = i;
    }
  }
  ...
  if (currentLevelIndex < 4) {
    progress = Math.min((totalPoints / nextLevelPoints) * 100, 100);
  }

  // Map to final badge array for rendering
