import React, { useState, useEffect } from "react"; // <-- IMPORT FIX
import {
...
const [scheduleTime, setScheduleTime] = useState("");
  ...
  useEffect(() => {
    const fetchPendingRequests = async () => {
      try {
        const res = await fetch('http://localhost:2007/api/requests/pending', { credentials: 'include' });
  ...
  // 2. handleAccept: Backend update karne ke liye
  const handleAccept = async (id) => {
    try {
      ...
      if (res.ok) {
        // Local state update karo UI refresh karne ke liye
        setRequests((prev) =>
    ...
    <div className="w-full max-w-[1400px] mx-auto px-5 md:px-8 lg:px-12 font-sans pb-10 -mt-2 sm:-mt-0">
      ...
      <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between w-full mb-8 ">
        ...
        <div className="flex flex-row w-full lg:w-auto gap-3 items-center">
          ...
          {/* Filter Button */}
          <button 
            className="h-10 px-4 md:px-5 rounded-2xl bg-white border border-gray-200 text-emerald-700 flex items-center gap-2 font-[600] text-sm shadow-sm hover:border-emerald-500 hover:text-emerald-700 transition-all shrink-0"
          >
        ...
        {requests.map((req) => (
          ...
          >
            ...
            <div className="flex gap-3">
              <img
                src={req.image?.startsWith('http') ? req.image : `http://localhost:2007${req.image}`} // <-- IMAGE FIX
                alt={req.wasteType}
            ...
            {/* ACTIONS */}
            <div className="mt-3">
              {req.status?.toLowerCase() === "pending" ? ( // <-- STATUS FIX
                <button
                  onClick={() => handleAccept(req._id)} // <-- ID FIX
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => handleCancel(req._id)} // <-- ID FIX
