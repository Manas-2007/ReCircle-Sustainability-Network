    ...
    <nav className="absolute top-0 left-0 w-full z-50">
      ...
      <div className="w-full max-w-[1500px] mx-auto px-6 lg:px-16 py-6 flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <div>
            <h2 className="text-xl md:text-3xl font-bold text-gray-900 leading-none tracking-tight">
              Re<span className="text-green-800">Circle</span>
            </h2>
            <p className="text-[9px] md:text-[10px] font-semibold tracking-widest text-black mt-0.5">
        ...
        <ul className="hidden lg:flex items-center gap-8">
          {navLinks.map((link, index) => (
            <li key={index}>
              <a
                href={
                  link === "How It Works" || link === "Contact Us"
                    ? undefined
                    : "#"
                }
        ...
        <div className="hidden lg:flex items-center">
          <button
            onClick={onOpenAuth}
            className="flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-[#16a34a] to-[#15803d] text-white font-medium rounded-full shadow-md hover:shadow-lg hover:scale-105 transition-all duration-300"
          >
      ...
      <div
        className={`fixed top-0 left-0 w-full bg-gradient-to-b from-green-50 via-white to-white shadow-2xl transition-transform duration-500 ease-in-out z-[60] rounded-b-3xl border-b-4 border-[#4ade80] ${isOpen ? "translate-y-0" : "-translate-y-full"}`}
      >
        {/* Close Button */}
        <button
        ...
        <div className="flex flex-col pt-7 pb-5 px-5">
          ...
          <div className="flex items-center gap-3 mb-10">
            ...
            />
            <div>
              <h2 className="text-2xl font-bold text-green-950 leading-none">
                Re<span className="text-green-800">Circle</span>
              </h2>
              <p className="text-[10px] font-bold text-[#15803d] tracking-widest mt-1">
                SUSTAINABILITY NETWORK
              </p>
            </div>
          </div>
          ...
          <ul className="flex flex-col items-start gap-5 w-full mb-10">
            {navLinks.map((link, index) => (
              <li key={index} className="w-full">
                ...
                >
                  {link}{" "}
                  <span className="text-[#4ade80] text-base drop-shadow-sm">
                    🍃
                  </span>
                </a>
              </li>
          ...
          <div className="w-full">
            <button
              onClick={onOpenAuth}
              className="flex items-center justify-center gap-2 w-full py-1.5 bg-gradient-to-r from-[#16a34a] to-[#15803d] text-white font-medium text-lg rounded-full shadow-md active:scale-95 transition-transform"
            >
