...
import { Wallet, ArrowUpRight, Recycle, Target, ArrowDownLeft, CreditCard, TrendingUp, Clock, X, Info } from 'lucide-react';
        ...
        <div className="relative p-5 sm:p-6">
          ...
          <div className="mb-5 rounded-2xl bg-gradient-to-r from-emerald-50 to-emerald-100/50 border border-emerald-100 p-4">
            <p className="text-sm text-gray-700 leading-relaxed">
              Rates may vary slightly depending on material quality and market conditions.
            </p>
          </div>
        ...
        <div className="relative p-5 sm:p-6">
          ...
          <div className="rounded-2xl bg-gray-50 p-4 mb-5">
            <p className="text-sm leading-relaxed text-gray-600">
              ...
              conversion factors. Different materials contribute differently
              to carbon reduction.
            </p>
          </div>
...
const Earnings = () => {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:2007/api/pricing-guide')
      ...
      .then(data => {
        setHistoryData(data);
        // Map history to transactions format
  ...
  const totalLifetimeEarnings = historyData.reduce((sum, item) => sum + (item.points || item.quantity * 10), 0);
  
  const avgPickup = historyData.length > 0 ? (totalLifetimeEarnings / historyData.length).toFixed(0) : 0;
...
const categoryStats = Object.entries(categoryTotals).map(([name, val]) => ({
  ...
  // Track pending payouts in state (simulated)
  ...
  const handleWithdrawal = () => {
    ...
    const newTransaction = {
      ...
      status: "Pending", // simulated as pending initially
      date: new Date().toLocaleDateString(),
      method: method.split(" ")[0], // e.g., "UPI" or "Bank"
      icon: <ArrowUpRight size={18} className="text-amber-600" /> // Use amber for pending
    };
...
const totalEarnings = currentWeekData.reduce((acc, item) => acc + (item.points || 0), 0);
const performancePercentage = Math.min(Math.round((wasteSaved / 80) * 100), 100);

  return (
    <div className="w-full max-w-[1400px] mx-auto px-5 md:px-8 lg:px-12 font-sans pb-10 -mt-2 sm:-mt-0 space-y-6">

      {/* 1. HERO SECTION: Balance & Buttons (Fully Responsive Wrapper Added) */}
      <div className="bg-emerald-900 text-white rounded-3xl p-6 md:p-8 shadow-xl flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div>
          ...
          <h1 className="text-3xl md:text-4xl font-bold mt-2">₹{balance.toFixed(2)}</h1>
          <p className="text-emerald-300 text-xs mt-2 flex items-center gap-1">
            <Clock size={12} /> Last payout: 25 May 2026
          </p>
        </div>
        
        {/* RESPONSIVE BUTTON CONTAINER: Stacks on mobile, side-by-side on desktop */}
      ...
      {/* MODALS */}
      <PricingModal isOpen={isPricingOpen} onClose={() => setIsPricingOpen(false)} pricingData={pricingData} />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setIsModalOpen(false)} className="p-1.5 hover:bg-gray-100 rounded-full"><X size={18}/></button>
            </div>
            <div className="space-y-4">
              ...
              >
                <option value="UPI (9876543210@ybl)">UPI (9876543210@ybl)</option>
                <option value="Bank Transfer (****1234)">Bank Transfer (****1234)</option>
              </select>
              <button 
      ...
      {/* 2. STATS GRID (DYNAMIC VALUES) */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { title: "This Month", value: `₹${thisMonthEarnings}`, icon: <TrendingUp size={20} />, color: "text-emerald-600" },
          { title: "Pending Payout", value: `₹${pendingPayout}`, icon: <Clock size={20} />, color: "text-amber-600" },
          { title: "Total Lifetime", value: `₹${totalLifetimeEarnings}`, icon: <Wallet size={20} />, color: "text-blue-600" },
          { title: "Avg. Pickup", value: `₹${avgPickup}`, icon: <CreditCard size={20} />, color: "text-purple-600" },
        ].map((stat, i) => (
          <div key={i} className="bg-white p-5 rounded-2xl border border-emerald-500 transition-colors shadow-sm">
            <div className={`mb-3 ${stat.color}`}>{stat.icon}</div>
            <p className="text-gray-500 text-[11px] font-bold uppercase tracking-wider">{stat.title}</p>
    ...
    <h3 className="font-bold text-gray-900 text-lg">Sustainability Impact</h3>
    <button onClick={() => setIsImpactModalOpen(true)} className="text-gray-400 hover:text-emerald-600 transition-colors">
       <Info size={18} />
    </button>
  </div>
          <div className="space-y-6">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-emerald-50 rounded-2xl text-emerald-600 border border-emerald-100"><Recycle size={24} /></div>
              <div>
                <p className="text-gray-900 font-bold">{historyData.reduce((acc, item) => acc + (item.quantity || 0), 0)} Kg Waste Diverted</p>
                <p className="text-xs text-gray-500 font-medium mt-0.5">From landfills to recycling units</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="p-3 bg-blue-50 rounded-2xl text-blue-600 border border-blue-100"><TrendingUp size={24} /></div>
              <div>
                <p className="text-gray-900 font-bold">{(historyData.reduce((acc, item) => acc + (item.quantity || 0), 0) * 0.24).toFixed(1)} Kg CO<sub>2</sub> Saved</p>
                <p className="text-xs text-gray-500 font-medium mt-0.5">Equivalent to planting 2 trees</p>
              </div>
            </div>
          </div>
        </div>
...
<div className="bg-white border border-gray-200 rounded-3xl p-6 shadow-sm">
  ...
  {categoryStats.length > 0 ? (
    ...
    <div className="text-center py-6 text-sm text-gray-400">No data available yet.</div>
  )}
</div>
      </div>

      {/* 4. NEW SECTION: TRANSACTIONS + GOAL */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-3xl border border-gray-300 shadow-sm">
          <h3 className="font-bold text-gray-900 mb-5 lg:text-lg">Recent Transactions</h3>
          
          {/* Scrollable Container */}
          <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
            {transactions.length > 0 ? transactions.map((t, index) => (
              <div key={t.id} className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-xl transition-all border border-transparent hover:border-gray-400 group">
                <div className="flex items-center gap-3">
                  {/* S.No */}
                  <span className="text-[12px] font-black text-gray-600 w-6">#{String(index + 1).padStart(2, '0')}</span>
                  
                  {/* Icon */}
                  <div className="p-2 bg-emerald-50 rounded-lg text-emerald-600">{t.icon}</div>
                ...
                <div className="flex flex-col items-end">
                   ...
                   {t.type === 'Payout' && (
                     <span className="text-[9px] font-bold text-amber-500 uppercase">{t.status}</span>
                   )}
                </div>
              </div>
            )) : (
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-emerald-500 via-emerald-600 to-emerald-700 p-5 sm:p-6 lg:p-7 text-white shadow-[0_10px_40px_rgba(16,185,129,0.25)] flex flex-col justify-between min-h-[220px] sm:min-h-[240px]">
          ...
          <div className="relative z-10">
            ...
            <p className="text-sm md:text-lg text-emerald-100/90 mt-2 max-w-xs leading-relaxed">
              Keep collecting to reach your weekly target and improve your impact.
            </p>
          </div>
