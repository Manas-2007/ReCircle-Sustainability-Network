...
const register = async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ message: 'Registration successful', user: { id: newUser._id, firstName: newUser.firstName, role: newUser.role } });
    } catch (err) {
        console.error(err);
...
const login = async (req, res) => {
    ...
    try {
        ...
        res.cookie('token', token, {
            httpOnly: true, secure: false, sameSite: 'lax', path:'/', maxAge: 86400000 
        });
...
const getProfile = async (req, res) => {
    try {
        ...
        if (!user) return res.status(404).json({ message: "User not found" });
        res.json({ points: user.points || 0, redeemedRewards: user.redeemedRewards || [] });
    } catch (err) {
        res.status(500).json({ error: "Server error" });
    }
};
