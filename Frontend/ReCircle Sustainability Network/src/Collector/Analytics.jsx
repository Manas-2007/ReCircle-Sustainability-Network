...
const Analytics = () => {
  ...
  const avgEfficiency =
    totalPickups > 0 ? (totalKg / totalPickups).toFixed(1) : 0;
  const uniqueCategories = [
    ...new Set(historyData.map((item) => item.wasteType)),
  ].length;
  ...
  const wasteStats = Object.keys(groupedWaste)
    ...
    .map((type, index) => {
      const val =
        totalKg > 0 ? Math.round((groupedWaste[type] / totalKg) * 100) : 0;
      return {
  ...
  const weeklyChartData = last7Days.map((date) => {
    ...
    const totalForDay = dayData.reduce(
      (sum, item) => sum + (item.quantity || 0),
      0,
    );
    ...
    <div className="w-full max-w-[1400px] mx-auto px-5 md:px-8 lg:px-12 font-sans pb-10 -mt-2 sm:-mt-0 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div
            key={i}
            className="bg-white p-4 md:p-6 rounded-3xl bg-green-50/50 border border-green-500 shadow-sm flex flex-col md:flex-row md:items-center gap-3 md:gap-4 transition-transform hover:-translate-y-1"
          >
            <div
              className={`${item.color} p-3 rounded-2xl text-white w-fit shrink-0`}
            >
              {item.icon}
            </div>
            <div>
              <p className="text-gray-600 text-[9px] md:text-[11px] uppercase font-bold tracking-wider mb-0.5">
                {item.label}
              </p>
        ...
        <div className="lg:col-span-2 bg-white p-5 md:p-6 rounded-3xl border border-gray-300 shadow-sm">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-[9px] md:text-[10px] text-gray-500 uppercase font-semibold mt-0.5">
                Weight Collected (KG) per Day
              </p>
            </div>
          ...
          <div className="flex items-end h-56 md:h-64 mt-4">
            ...
            <div className="flex items-end justify-center md:justify-between gap-1.5 md:gap-0 w-full h-full border-b border-l border-gray-400 pb-2 px-2">
              {weeklyChartData.map((item, i) => (
                <div
                  key={i}
                  className="flex flex-col items-center justify-end h-full group w-7 md:w-12"
                >
                  {/* Peak Label */}
                  ...
                  <div
                    ...
                    style={{
                      height: `${Math.min((item.val / 100) * 100, 100)}%`,
                    }}
                  />

                  {/* X-Axis Label */}
                  <span className="text-[8px] md:text-[10px] font-semibold text-gray-600 mt-2.5 md:mt-3">
                    {item.day}
                  </span>
                </div>
              ))}
