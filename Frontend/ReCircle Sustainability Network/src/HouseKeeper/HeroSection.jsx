        ...
        <div className="xl:col-span-3 space-y-6 font-sans">
          ...
          <div className="bg-white rounded-3xl border border-gray-300 p-3 sm:p-6 shadow-sm grid grid-cols-2 lg:grid-cols-4 gap-y-4 sm:gap-y-6 gap-x-3">
            {stats.map((stat, i) => (
              <div
                key={i}
                className={`flex items-center gap-2.5 sm:gap-4 px-1 sm:px-2 min-w-0 w-full ${i !== 3 ? "lg:border-r border-gray-300" : ""}`}
              >
                ...
                <div className="flex flex-col min-w-0 flex-1">
                  ...
                  <span className="text-[9px] sm:text-[11px] font-medium text-gray-400 mt-0.5 leading-tight">
                    {stat.sub}
                  </span>
                </div>
              </div>
            ...
            <div className="mt-4">
              {requests
                ...
                .map((req, i) => {
                  ...
                  const collectorName =
                    typeof req.collector === "string"
                      ? req.collector
                      : req.collector?.name || "ReCircle Partner";

                  return (
                    <div
                      key={req._id || i}
                      className="flex flex-col lg:flex-row items-start lg:items-center justify-between p-3 sm:p-5 bg-white rounded-3xl border border-gray-200 shadow-sm hover:shadow-md transition-all duration-300 gap-3 sm:gap-6 mb-4 group"
                    >
                      {/* LEFT: IMAGE & DETAILS */}
                      <div className="flex w-full lg:w-auto gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-2 mb-1">
                            ...
                            <span
                              className={`px-1.5 py-0.5 rounded-md text-[7px] sm:text-[9px] font-bold uppercase shrink-0 border ${isAssigned ? "bg-emerald-100 text-emerald-700 border-emerald-300" : "bg-amber-100 text-amber-700 border-amber-300"}`}
                            >
                              {req.status}
                            </span>
                          </div>
          ...
          <div className="bg-white p-4 sm:p-6 rounded-3xl border border-gray-300 shadow-sm w-full font-sans mt-6">
            ...
            <div className="flex justify-between items-center mb-4 sm:mb-6">
              ...
              <Link
                ...
                className="text-[10px] sm:text-xs font-semibold text-emerald-600 hover:text-emerald-700 bg-emerald-50 hover:bg-emerald-100 px-2.5 sm:px-3 py-1.5 rounded-lg transition-colors"
              >
                View All
              </Link>
            </div>
              ...
              <table className="w-full min-w-[500px] sm:min-w-[600px] text-left border-collapse">
                <thead>
                  <tr className="border-b border-gray-300">
                    <th className="pb-3 text-[9px] sm:text-[11px] font-bold text-gray-400 uppercase tracking-wider pl-2">
                      S.No
                    </th>
                    ...
                    <th className="pb-3 text-[9px] sm:text-[11px] font-bold text-gray-400 uppercase tracking-wider text-right pr-2">
                      Status
                    </th>
                  </tr>
                </thead>
                  ...
                  {requests.slice(0, 4).map((item, i) => (
                    ...
                    >
                      ...
                      <td className="py-2.5 sm:py-3 pr-3">
                        <span className="font-semibold sm:font-bold text-gray-800 text-[11px] sm:text-[14px] capitalize">
                          {item.wasteType}
                        </span>
                      </td>
                      <td className="py-2.5 sm:py-3 pr-3 text-[11px] sm:text-[14px] font-medium text-gray-600">
                        {item.quantity} kg
                      </td>
                      <td className="py-2.5 sm:py-3 pr-3">
                        <div className="flex items-center gap-1 bg-emerald-50 text-emerald-700 w-fit px-1.5 sm:px-2 py-1 rounded-md">
                          ...
                          <span className="text-[10px] sm:text-[14px] font-bold">
                            +{item.points || 0}
                          </span>
                        </div>
                      </td>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-br from-[#f2fdf5] to-white p-5 sm:p-6 rounded-3xl border border-emerald-300/80 shadow-sm w-full transition-all duration-300 ease-out">
            ...
            <div className="flex items-center gap-4 sm:gap-6">
              ...
              <p className="text-xs sm:text-[13px] text-gray-600 font-medium leading-relaxed">
                You've recycled{" "}
                <strong className="text-emerald-800">{totalWaste} kg</strong>{" "}
                waste, saving{" "}
                <strong className="text-emerald-800">{co2Saved} kg</strong> of
                CO2 emissions! 🌿
              </p>
            </div>
          </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                  ...
                  <div className="space-y-4">
                    <p className="text-sm font-semibold text-gray-700 leading-relaxed">
                      Every kilogram of waste you recycle helps reduce the
                    ...
                    <div className="space-y-3">
                      {[
                        {
                          icon: "🌍",
                          text: "Preventing harmful methane release from landfills.",
                        },
                        {
                          icon: "⚡",
                          text: "Saving energy needed for virgin material production.",
                        },
                        {
                          icon: "🏭",
                          text: "Reducing industrial emission levels globally.",
                        },
                      ].map((item, i) => (
                        <div
                          key={i}
          ...
          <div className="relative w-full rounded-3xl overflow-hidden font-sans min-h-[300px] p-7 sm:p-5 flex flex-col justify-between shadow-md hover:-translate-y-1 hover:shadow-xl transition-all duration-300 group cursor-pointer">
            ...
            <div className="relative z-10 flex flex-col h-full justify-between">
              ...
              <div className="flex justify-between items-start mb-3">
                ...
                <Link
                  to="/housekeeper/eco"
                  className="text-[10px] sm:text-xs font-semibold text-white/90 border border-white/20 bg-white/10 rounded-lg px-3 py-1.5 hover:bg-white/20 transition-all backdrop-blur-sm"
                >
                  View All
                </Link>
              </div>
              ...
              <div>
                <Link
                  to="/housekeeper/eco"
                  className="inline-flex items-center justify-center gap-2 bg-[#438a4f] text-white py-2.5 px-4 sm:px-5 rounded-xl text-xs sm:text-sm font-[600] border border-[#52a65f] shadow-lg hover:bg-[#387642] hover:shadow-xl hover:gap-3 transition-all duration-300"
                >
          ...
          <div className="bg-gradient-to-br from-emerald-900 to-[#064e3b] p-6 sm:p-7 rounded-3xl border border-emerald-800 shadow-xl font-sans w-full relative overflow-hidden group hover:-translate-y-1 transition-transform duration-300">
            <div className="absolute -right-12 -top-12 opacity-10 group-hover:opacity-20 transition-opacity duration-700 pointer-events-none">
              <Globe
                size={180}
                className="text-emerald-100 animate-[spin_20s_linear_infinite]"
                strokeWidth={1}
              />
            </div>
            ...
            <div className="relative z-10 flex flex-col items-center text-center">
              <div className="relative mb-5">
                ...
                <div className="w-14 h-14 sm:w-16 sm:h-16 bg-white/10 backdrop-blur-md rounded-full flex items-center justify-center border border-white/20 shadow-[0_0_15px_rgba(16,185,129,0.2)]">
                  <Globe
                    size={28}
                    className="text-emerald-100 animate-[spin_8s_linear_infinite] sm:w-8 sm:h-8"
                  />
                </div>
                <Sparkles
                  size={16}
                  className="absolute -top-1 -right-2 text-emerald-300 animate-bounce"
                />
              </div>
              ...
              <p className="text-emerald-100/80 text-xs sm:text-[13px] leading-relaxed font-medium mb-6 max-w-[220px]">
                Every item you recycle is a step towards a greener, cleaner
                Earth. You're part of a global movement today.
              </p>
              <button
                onClick={() => setShowCo2Modal(true)}
                className="w-full text-[11px] sm:text-xs font-bold text-emerald-950 bg-emerald-100 px-4 py-2.5 rounded-xl hover:bg-white hover:shadow-[0_0_15px_rgba(255,255,255,0.4)] transition-all duration-300"
              >
                See Global Impact
              </button>
            </div>
